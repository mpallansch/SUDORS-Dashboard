...
const inputFilePath = './SUDORS 2020 Prelim Data for Dashboard_22FEB2022.csv';
const outputPathRoot = '../public/data/';
const typeOfDrugFilePath = outputPathRoot + 'causes.json';
const additionalDrugFilePath = outputPathRoot + 'additional-drugs.json';
const circumstancesFilePath = outputPathRoot + 'circumstances.json';
const mapFilePath = outputPathRoot + 'map.json';
const sexFilePath = outputPathRoot + 'sex.json';
const ageFilePath = outputPathRoot + 'age.json';
const ageBySexFilePath = outputPathRoot + 'age-by-sex.json';
const raceFilePath = outputPathRoot + 'race.json';
const stateFilePath = outputPathRoot + 'state.json';
const interventionFilePath = outputPathRoot + 'interventions.json';
const totalsFilePath = outputPathRoot + 'totals.json';
const timeFilePath = outputPathRoot + 'time.json';
const opioidStimulantFilePath = outputPathRoot + 'opioid-stimulant.json';
const drugCombinationFilePath = outputPathRoot + 'drug-combination.json';
const ageAdjustedRatesFilePath = outputPathRoot + 'age-adjusted-rates.json';
const ageAdjustedSexRatesFilePath = outputPathRoot + 'age-adjusted-sex-rates.json';
const ageAdjustedRaceRatesFilePath = outputPathRoot + 'age-adjusted-race-rates.json';
const ageAdjustedDrugRatesFilePath = outputPathRoot + 'age-adjusted-drug-rates.json';
      ...
      circumstancesData[state] = {
        intervention: [
          { 
            ...
            percent: percent(keyCounts[state]['recentinst']['1'], keyCounts[state]['combo_cme_flag']['0']),
            count: checkCutoff(keyCounts[state]['recentinst']['1']) },
          { 
            circumstance: 'Prior overdose', 
            percent: percent( keyCounts[state]['priorod']['1'], keyCounts[state]['combo_cme_flag']['0']),
            count: checkCutoff(keyCounts[state]['priorod']['1']) },
          { 
            circumstance: 'Mental health diagnosis', 
            percent: percent(keyCounts[state]['MentalHealthProblem_c']['1'], keyCounts[state]['combo_cme_flag']['0']),
            count: checkCutoff(keyCounts[state]['MentalHealthProblem_c']['1']) },
          { 
            circumstance: 'Current treatment for substance use disorder', 
            percent: percent(keyCounts[state]['currenttx']['1'], keyCounts[state]['combo_cme_flag']['0']),
            count: checkCutoff(keyCounts[state]['currenttx']['1']) },
          { 
            circumstance: 'Potential bystander present', 
            percent: percent(keyCounts[state]['bystander']['1'], keyCounts[state]['combo_cme_flag']['0']),
            count: checkCutoff(keyCounts[state]['bystander']['1']) },
          { 
            circumstance: 'Fatal drug use witnessed', 
            percent: percent(keyCounts[state]['witnesseddruguse']['1'], keyCounts[state]['combo_cme_flag']['0']),
            count: checkCutoff(keyCounts[state]['witnesseddruguse']['1']) },
        ],
        other: [
          { 
            circumstance: 'History of substance use/misuse', 
            percent: percent(keyCounts[state]['SubstanceAbuseOther_c']['1'], keyCounts[state]['combo_cme_flag']['0']),
            count: checkCutoff(keyCounts[state]['SubstanceAbuseOther_c']['1']) },
          { 
            circumstance: 'Naloxone administered', 
            percent: percent(keyCounts[state]['NaloxoneAdministered']['1'], keyCounts[state]['combo_cme_flag']['0']),
            count: checkCutoff(keyCounts[state]['NaloxoneAdministered']['1']) },
          { 
            circumstance: 'Current pain treatment', 
            percent: percent(keyCounts[state]['pain_treat']['1'], keyCounts[state]['combo_cme_flag']['0']),
            count: checkCutoff(keyCounts[state]['pain_treat']['1']) },
          { 
            circumstance: 'Recent return to use of opioids', 
            percent: percent(keyCounts[state]['recentrelapse']['1'], keyCounts[state]['combo_cme_flag']['0']),
            count: checkCutoff(keyCounts[state]['recentrelapse']['1']) },
          { 
            circumstance: 'Experiencing homelessness', 
            percent: percent(keyCounts[state]['Homeless']['1'], keyCounts[state]['combo_cme_flag']['0']),
