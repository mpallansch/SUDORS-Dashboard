        ...
        <Group top={margin.top} left={margin.left}>
          {data[metric].map(d => (
              <Group key={`group-${d.circumstance}`}>
                ...
                {d.circumstance === 'Current treatment for substance use disorder(s)' && <text aria-describedby="footnote-k" width={adjustedWidth} x={380} y={yScale(d.circumstance) + barThickness + margin.bar + 5} verticalAnchor="start" fontSize="13">j</text>}
                {d.circumstance === 'Potential bystander present' && <text aria-describedby="footnote-j" width={adjustedWidth} x={225} y={yScale(d.circumstance) + barThickness + margin.bar + 5} verticalAnchor="start" fontSize="13">k</text>}
